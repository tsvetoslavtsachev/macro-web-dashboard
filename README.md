...
## O3/O4 конвенция (Вълна 1, 2026-07)

- **Каноничен прозорец = 10г** (default). При него percentile-ът е JSON `latest.percentile` (ТОЧНО api/briefing числото) с етикет „(10г канон)"; при мръднат слайдер — клиентско преизчисление с етикет „(изследователски прозорец Xг)". Числото никога не сменя стойност мълчаливо.
- **O4 бадж** до заглавието: „НАБЛЮДЕНИЕ, НЕ СИГНАЛ · coincident" — уредът описва състояние, не дава сигнал.
